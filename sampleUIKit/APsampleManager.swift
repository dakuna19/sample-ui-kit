...
/** APsampleManager gives an easy access to Anima code */
public class APsampleManager : NSObject {

    public static let sharedInstance = APsampleManager();

    public static func shared() -> APsampleManager {
        return self.sharedInstance
    }

    /**
    Holds the theme as configured in Anima.
    On runtime, you may change its values and call APsampleManager.sharedInstance().theme.apply()
    **/
    ...
    public func animaStoryboad() -> UIStoryboard {
        return UIStoryboard(name: "sampleUIKit", bundle:Bundle(for: type(of: self).self));
    }
